...
# 1️⃣ Load secrets from environment variables
stripe.api_key = os.getenv("STRIPE_SECRET_KEY")
WEBHOOK_SECRET = os.getenv("STRIPE_WEBHOOK_SECRET")
LICENSE_SECRET = os.getenv("LICENSE_SECRET")

if not stripe.api_key or not WEBHOOK_SECRET or not LICENSE_SECRET:
    raise RuntimeError(
        "Environment variables STRIPE_SECRET_KEY, STRIPE_WEBHOOK_SECRET, LICENSE_SECRET must be set."
    )

# 2️⃣ File to store users
USERS_FILE = "users.json"

# 3️⃣ Map tier → live Stripe Price IDs
TIER_PRICE_IDS = {
    "pro": os.getenv("PRICE_PRO_ID"),        # Set in Render
    "diamond": os.getenv("PRICE_DIAMOND_ID") # Set in Render
}
